...
{
  ROS_INFO_STREAM("Starting sick LMS node with arguments 1,2 as ["<< argv[1] << "," << argv[2] <<"].");
  ros::init(argc, argv, "SICK_sensor");
  ros::NodeHandle n;
	std::string myTopicName;
	myTopicName="LMS_"+std::string(argv[1]);
	ROS_INFO_STREAM("Publishing topic ["<< myTopicName <<"]");
  ros::Publisher publisher = n.advertise<sensor_msgs::LaserScan>(myTopicName,100);
  ...
  n.param("sick/rate", freq, 50.0);
	...
	// modify hostname IP to match last byte from args  
  ...
  do {
    ROS_INFO_STREAM("Attempting to connect to device LMS100 on IP address: ["<<hostname<<"]" );
    if ( lms->Connect() ) {  
